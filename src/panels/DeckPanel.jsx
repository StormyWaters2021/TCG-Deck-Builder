  ...
  if (moveMode === "one") {
    // move exactly one copy
    onRemoveCard(cardId, 1, fromSection);
    onAddCard   (cardId, 1, toSection);
  } else {
    // move all copies that live in the source section
    const entry = deck[cardId] || {};
    const bucketCount =
      entry.group && entry.group[fromSection] != null
        ? entry.group[fromSection]
        : entry.count || 0;
    if (bucketCount > 0) {
      onRemoveCard(cardId, bucketCount, fromSection);
      onAddCard   (cardId, bucketCount, toSection);
    }
  }
}
	  ...
	  const processedDeckByName = {};
		Object.entries(deck).forEach(([cardId, entry]) => {
		  const card = cards.find(c => c.id === cardId);
		  if (!card) return;
		  const count = entry.count || 0;              // <-- get the real number
		  const name = card.name;
		  if (!processedDeckByName[name]) {
			processedDeckByName[name] = { qty: 0, cardIds: [], card };
		  }
		  processedDeckByName[name].qty += count;      // <-- add the number
